...
class DashboardController extends Controller
{
    ...
    {
        // return ($request->openExternally);
        $validate = Validator::make($request->all(), [
            'feature' => 'required',
            'fileImg' => 'file|mimes:jpg,png,pdf,mp4|max:250000',
            'appPackage' => $request->openExternally == 'on' ? 'required' : '',
        ]);
        ...
        if($existingCompany && $existingCompany->is_delete === "1"){
            ...
            $existingCompany->app_package = $request->appPackage;
            $existingCompany->is_active = ($request->has('status') && $request->status == 'on') ? '1' : '0';
            $existingCompany->open_externally = isset($request->openExternally) && $request->openExternally == 'on' ? '1': '0';
            ...
            $company->app_package = $request->appPackage;
            $company->is_active = ($request->has('status') && $request->status == 'on') ? '1' : '0';
            $company->open_externally = isset($request->openExternally) && $request->openExternally == 'on' ? '1'
          : '0';
    ...
    public function updateFeature(Request $request)
    {
        ...
        $updateCompany->app_package = $request->edit_app_package;
        $updateCompany->is_active = ($request->has('status') && $request->status == 'on') ? '1' : '0';
        $updateCompany->open_externally = isset($request->edit_open_externally) && $request->edit_open_externally == 'on' ? '1': '0';
        $updateCompany->save();
        if ($updateCompany) {
    ...
    {
         if($request->confirm_user_id)
         {
         $user = User::find($request->confirm_user_id);
         if (!$user) {
        ...
        if ($request->has('edit_user_id') && $request->edit_user_id != null) {
           ...
           if ($validate->fails()) {
            return response()->json(['success' => false,'errors' => $validate->errors()->toArray() ], 422);
           }
            ...
            $existingUserEmail = User::where('email', $request->user_email)->first();
            if ($existingUserEmail && $existingUserEmail->is_delete == '1' && $existingUserEmail->is_active == '0') {
                return response()->json(['success' => 'deletedAcount', 'message' => $existingUserEmail], 200);
            }
            ...
            $saveuser->save();
            if ($saveuser) {
                return response()->json(['success' => 'userAdded', 'id' => $saveuser->id], 200);
            } else {
    ...
    public function testerQuizzes()
    {
        ...
        ->orderBy('sub_categories.priority', 'asc')->get();
         $region = Region::where('is_deleted', 'N')->where('is_active','1')->get();
        return view('admin.tester-quiz', compact('testerQuizzes', 'getCategories','region'));
    }
    ...
    public function getSubCategoryRegion(Request $request)
    {
        return $request;
        $categories = SubCategory::where('is_delete','0')->where('is_active','1')
        ->where('category_id', $request->category_id)
    ...
    public function saveTesterQuizzes(Request $request)
    {
        ...
        if($existingSubCategory !=null && $existingSubCategory->is_delete === "1"){
            ...
            $saveSubCategory->save();
             ...
             for($j = 0; $j < count($request->region_id); $j++){
                    $sub_category_region = new SubCategoryRegion();
                    $sub_category_region->sub_category_id = $saveSubCategory->id;
                    $sub_category_region->region_id = $request->region_id[$j];
                    $sub_category_region->save();
                }
            if ($saveSubCategory) {
                Session::flash('success', 'Sub Category Added Successfully');
                Session::flash('alert-class', 'alert-success');
    ...
    public function updateSubCategory(Request $request){
        ...
        if ($updateSubCategory) {
             $deletePrevious = SubCategoryRegion::where('sub_category_id',$updateSubCategory->id)->get();
                foreach($deletePrevious as $delete){
                    $delete->delete();
                }
                for($j = 0; $j < count($request->edit_region_id); $j++){
                   $subCategoryRegion = new SubCategoryRegion();
                   $subCategoryRegion->sub_category_id = $updateSubCategory->id;
                   $subCategoryRegion->region_id = $request->edit_region_id[$j];
                   $subCategoryRegion->save();
                }
    ...
    public function editTesterQuizzes(Request $request)
    {
        ...
        $region = Region::where('is_deleted' ,'N')->where('is_active','1')->get();
        $regionID = SubCategoryRegion::where('sub_category_id', $request->sub_category_id)->pluck('region_id');
        $relatedRegions = Region::whereIn('id', $regionID)->select('region_name','id')->get();

        return response()->json(['success' => '1', 'data' => $getQuizData, 'category' =>
        $getCategories,'related_regions'=> $relatedRegions,'region'=> $region]);
    }
    ...
    public function saveFile(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->file('image') as $file) {
                ...
                if (!empty($file)) {
                    foreach ($regionIds as $regionValue) {
                        $newFileCon = new FileContent();
                        $newFileCon->file_id = $newFile->id;
                        $newFileCon->file_path = $originalFileName; // Adjust as needed
                        $newFileCon->file_type_id = $request->file_type_id;
                        $newFileCon->region_id = $regionValue;
                        $newFileCon->save();
                    }
                }
            }
    ...
    public function subCatChild()
    {
        ...
        ->get();
        return view('admin.child', compact('testerQuizzes', 'getCategories', 'sub_cat_child','region'));
    }

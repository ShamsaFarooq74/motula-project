...
class StripePaymentController extends Controller
{
    public function stripe($id)
    {
        ...
        $getStripePublishKey = Setting::where('perimeter','Stripe_Key')->first()['value'];
        return view('web.stripe',compact('Courses','currencySymbol','getStripePublishKey'));
    }
...
//            $MAC = exec('getmac');
            ...
            $price = 100 * $request->course_price;
            $charge = Stripe\Charge::create ([
                "amount" => $price,
                "currency" => "gbp",
                "source" => $request->stripeToken,
                "description" => "SERU Training Online Courses"
            ]);

            $newTransaction = new UserCourseTransactions();
            $newTransaction->user_id = Auth::user()->id;
            $newTransaction->course_id = $request->course_id;
            $newTransaction->account_number =$request->card_number;
            $newTransaction->account_holder_name = $request->account_name;
            $newTransaction->account_cvc = "";
            $newTransaction->amount_paid = $request->course_price;
            $newTransaction->stripe_token = $request->stripeToken;
            $newTransaction->expiry_month = $request->card_expiry_month;
            $newTransaction->expiry_year = $request->card_expiry_year;
            $newTransaction->stripe_response = $charge;
            $newTransaction->ip = $ip;
            $newTransaction->mac_address = $macAddress;
            $newTransaction->browser = $browserInfo['browser'];
            $newTransaction->device_type = $browserInfo['device'];
            $newTransaction->os = $browserInfo['os_platform'];
            $newTransaction->save();

            //after this add an entry in user_course table
            ...
            if($newTransaction){
                ...
                $addCourse->end_date = $currentDate->addWeeks($Courses->duration);
                $addCourse->save();
                if($addCourse){
                    Session::flash('success', 'Payment successful! You have successfully joined the course');
